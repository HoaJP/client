...
function TeacherAssignments() {
  ...
  const handleDelete = async (classId) => {
    if (window.confirm("Bạn có chắc muốn hủy đăng ký lớp học này?")) {
      try {
        await availabilityService.deleteAssignment(teacherId, classId);
        const registeredResponse =
          await availabilityService.getTeacherAssignments(teacherId);
        setRegisteredClasses(registeredResponse.data);
        ...
        setSelectedClassDetails(null);
        setNewAssignment({ ...newAssignment, class_id: "" });
        setError(null);
      } catch (err) {
        setError(err.response?.data?.error || "Không thể hủy đăng ký lớp học");
      }
    }
  };
              ...
              <form onSubmit={handleSubmit}>
                ...
                {selectedClassDetails && (
                  <div className="mb-3">
                    ...
                    {selectedClassDetails.schedules?.length > 0 ? (
                      <div className="table-responsive">
                        <table className="table table-striped">
                          ...
                          <tbody>
                            {selectedClassDetails.schedules.map(
                              (schedule, index) => (
                                <tr key={index}>
                                  <td>{selectedClassDetails.class_name}</td>
